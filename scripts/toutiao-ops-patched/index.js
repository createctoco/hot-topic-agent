...
publish
  ...
  .option('--cover <path>', '封面图片路径（单图模式，使用 --cover-free 时可不填）')
  .option('--cover-mode <mode>', '封面模式: single / triple / none / free', 'single')
  .option('--cover-free', '使用头条免费图库自动选图')
  .option('--cover-keyword <keyword>', '免费图库搜索关键词（默认使用文章标题关键词）')
  ...
  .option('--headless', '无头模式运行')
  .action(async (opts) => {
    // 如果指定了 --cover-free，将 coverMode 设为 'free'
    if (opts.coverFree) {
      opts.coverMode = 'free';
    }
